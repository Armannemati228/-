...
import { History as HistoryIcon, Filter, Printer, Search, Calendar, User } from 'lucide-react';

export const History: React.FC = () => {
  const { systemLogs, users, currentUser } = useApp();
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col md:flex-row justify-between items-center gap-4 bg-white dark:bg-gray-800 p-6 rounded-2xl shadow-sm border border-gray-100 dark:border-gray-700 print:hidden">
          <div><h2 className="text-2xl font-bold text-gray-800 dark:text-white flex items-center gap-2"><HistoryIcon className="text-blue-600"/> تاریخچه رفتار کاربران</h2><p className="text-sm text-gray-500 mt-1">گزارش کامل عملیات و تغییرات انجام شده در سیستم</p></div>
          <button onClick={handlePrint} className="flex items-center gap-2 px-4 py-2 bg-blue-100 text-blue-700 rounded-xl hover:bg-blue-200 transition-colors font-bold"><Printer size={18}/> چاپ گزارش</button>
      </div>
      ...
      <div className="bg-white dark:bg-gray-800 rounded-2xl shadow-sm border border-gray-100 dark:border-gray-700 overflow-hidden print:border-2 print:border-black">
          <table className="w-full text-right text-sm">
              ...
              <tbody className="divide-y dark:divide-gray-700">
                  ...
                  {filteredLogs.length === 0 && (<tr><td colSpan={6} className="p-8 text-center text-gray-400">هیچ رکوردی با فیلترهای انتخاب شده یافت نشد.</td></tr>)}
              </tbody>
          </table>
      </div>
    </div>
