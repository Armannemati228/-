...
export const Dashboard: React.FC = () => {
  const { financials, users, dogs, currentUser, addTrainerToDog, services, addTrainingSession, rooms, 
    managerTasks, dailyChecklists, emergencyReports, systemLogs, submitChecklist, verifyChecklist, addEmergencyReport, assignEmergencyReport, resolveEmergencyReport,
  ...
  const [viewChecklistModal, setViewChecklistModal] = useState<DailyChecklist | null>(null);
  ...
  const pendingChecklists = dailyChecklists.filter(c => !c.verifiedByAdmin); // New filter for Admin
  ...
  const handleVerifyChecklist = () => { if(viewChecklistModal) { verifyChecklist(viewChecklistModal.id); setViewChecklistModal(null); alert('گزارش تایید و در آرشیو ذخیره شد.'); } };
  ...
  return (
    <div className="space-y-6 pb-20">
      ...
      {/* --- CLIENT DASHBOARD --- */}
      {isClient && (/* ... client dashboard ... */ <div className="grid grid-cols-1 lg:grid-cols-2 gap-6"><div className="bg-white dark:bg-gray-800 rounded-2xl p-6 shadow-sm border border-gray-100 dark:border-gray-700"><h3 className="font-bold dark:text-white mb-4 flex items-center gap-2"><CreditCard size={20} className="text-indigo-500"/> صورت‌حساب‌های من</h3><div className="space-y-3 max-h-96 overflow-y-auto">{myPendingInvoices.map(inv => { const remaining = inv.finalAmount - (inv.paidAmount || 0); return (<div key={inv.id} className="p-4 bg-indigo-50 dark:bg-indigo-900/20 rounded-xl border border-indigo-100 dark:border-indigo-800"><div className="flex justify-between items-start mb-2"><div><p className="font-bold text-sm text-indigo-900 dark:text-indigo-200">{inv.serviceName || 'خدمات باشگاه'}</p><p className="text-xs text-indigo-600 dark:text-indigo-300">{inv.date}</p></div><span className="bg-white dark:bg-gray-800 text-indigo-600 px-2 py-1 rounded text-xs font-bold shadow-sm">بدهی: {new Intl.NumberFormat('fa-IR').format(remaining)} ت</span></div><div className="flex justify-between items-center"><div className="text-xs text-gray-500">کل: {new Intl.NumberFormat('fa-IR').format(inv.finalAmount)} | پرداخت شده: {new Intl.NumberFormat('fa-IR').format(inv.paidAmount || 0)}</div><button onClick={() => { setShowPaymentModal(inv); setPaymentForm({ amount: remaining.toString(), method: PaymentMethod.ONLINE }); }} className="bg-indigo-600 text-white px-4 py-1.5 rounded-lg text-xs font-bold hover:bg-indigo-700 transition-colors">پرداخت</button></div></div>); })}{myPendingInvoices.length === 0 && <p className="text-sm text-gray-400 text-center py-8">شما هیچ فاکتور پرداخت نشده‌ای ندارید.</p>}</div></div><div className="space-y-6"><div className="bg-gradient-to-br from-green-500 to-emerald-600 rounded-2xl p-6 text-white shadow-lg"><div className="flex justify-between items-start mb-4"><div><p className="text-emerald-100 text-sm mb-1">موجودی کیف پول شما</p><h3 className="text-3xl font-black">{new Intl.NumberFormat('fa-IR').format(currentUser?.balance || 0)} <span className="text-sm font-normal">تومان</span></h3></div><div className="bg-white/20 p-2 rounded-xl"><Wallet size={24}/></div></div><button className="w-full py-2 bg-white text-green-600 rounded-xl font-bold text-sm hover:bg-gray-50 transition-colors">افزایش موجودی (شارژ)</button></div><div className="bg-white dark:bg-gray-800 rounded-2xl p-6 shadow-sm border border-gray-100 dark:border-gray-700"><h3 className="font-bold dark:text-white mb-4 flex items-center gap-2"><Bone size={20} className="text-amber-500"/> سگ‌های من</h3><div className="space-y-2">{myDogs.map(d => (<div key={d.id} className="flex items-center gap-3 p-2 hover:bg-gray-50 dark:hover:bg-gray-700 rounded-lg transition-colors cursor-pointer" onClick={() => navigate('/dogs')}><img src={d.image} className="w-10 h-10 rounded-full object-cover border border-gray-200 dark:border-gray-600" /><div><p className="text-sm font-bold dark:text-white">{d.name}</p><p className="text-xs text-gray-500">{d.status}</p></div></div>))}</div></div></div></div>)}
      ...
      {viewChecklistModal && (
          <div className="fixed inset-0 bg-black/60 z-50 flex items-center justify-center p-4 backdrop-blur-sm">
              <div className="bg-white dark:bg-gray-800 rounded-2xl p-6 max-w-lg w-full animate-fade-in shadow-2xl">
                  <div className="flex justify-between items-center mb-4"><h3 className="font-bold text-lg dark:text-white">بازبینی گزارش روزانه مدیر داخلی</h3><button onClick={() => setViewChecklistModal(null)}><XCircle className="text-gray-500"/></button></div>
                  <div className="bg-gray-50 dark:bg-gray-700/30 p-3 rounded-lg mb-4 text-sm"><p><strong>تاریخ:</strong> {viewChecklistModal.date} | {viewChecklistModal.submissionTime}</p><p><strong>مدیر ثبت کننده:</strong> {viewChecklistModal.managerName}</p></div>
                  <div className="max-h-60 overflow-y-auto space-y-2 mb-6">
                      {viewChecklistModal.items.map((item, i) => (
                          <div key={i} className="flex justify-between items-center p-2 border-b dark:border-gray-700 last:border-0">
                              <span className={`text-sm ${item.isDone ? 'text-green-600 dark:text-green-400' : 'text-red-500'} flex items-center gap-2`}>
                                  {item.isDone ? <CheckCircle size={14}/> : <XCircle size={14}/>} {item.title}
                              </span>
                              {item.note && <span className="text-xs text-gray-500 bg-gray-100 dark:bg-gray-800 px-2 py-0.5 rounded">{item.note}</span>}
                          </div>
                      ))}
                  </div>
                  <button onClick={handleVerifyChecklist} className="w-full py-3 bg-purple-600 text-white rounded-xl font-bold hover:bg-purple-700 flex items-center justify-center gap-2"><Archive size={18}/> تایید و بایگانی در تاریخچه</button>
              </div>
          </div>
      )}
